/// Parsing for `cybercare://` deep links.
///
/// Mirrors the desktop handler in `desktop/src-tauri/src/deep_link.rs`:
/// `cybercare://message?channel=<uuid>&id=<hex>[&thread=<hex>]` references a
...
/// `cybercare://join?relay=<ws(s)://relay>&code=<code>` form is only an
/// installed-app handoff from the web landing page.
...
///
/// Canonical form: `cybercare://channel/<channel-uuid>`.
...
/// A parsed `cybercare://message` deep link.
...
/// Build a canonical `cybercare://message` link for a channel message.
///
...
/// Parse a canonical `cybercare://channel/<channel-uuid>` URI.
///
...
ChannelDeepLink? parseChannelDeepLink(Uri uri) {
  if (uri.scheme != brand.deepLinkScheme || uri.host != 'channel') return null;
  if (uri.hasQuery ||
      uri.hasFragment ||
...
/// Parse a `cybercare://message?…` URI into a [MessageDeepLink].
///
...
MessageDeepLink? parseMessageDeepLink(Uri uri) {
  if (uri.scheme != brand.deepLinkScheme || uri.host != 'message') return null;
...
/// Parse canonical HTTPS invite links and `cybercare://join` app handoffs.
///
/// Accepted forms:
/// - `https://<relay>/invite/<code>` -> `wss://<relay>` + code
/// - `http://localhost/invite/<code>` -> `ws://localhost` + code in debug builds
/// - `cybercare://join?relay=<wss://relay>&code=<code>` -> relay + code
/// - `cybercare://join?relay=<ws://localhost>&code=<code>` -> local relay in
///   debug
///
...
/// A validated repository, pull request, or issue permalink.
...
/// Parse canonical `cybercare://repo|pr|issue` permalinks for inline
/// presentation.
