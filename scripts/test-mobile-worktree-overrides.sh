...
grep -q '^BUNDLE_IDENTIFIER = africa\.cybota\.cybercare\.commons\.feature-work-1$' "$ios" \
...
grep -q '^APP_DISPLAY_NAME = Commons (Fix_Thing-2)$' "$ios" \
...
grep -Eq "^APP_DISPLAY_NAME = Commons \([A-Za-z0-9._-]+\)$" "$ios" \
...
grep -q '^BUNDLE_IDENTIFIER = africa\.cybota\.cybercare\.commons$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production bundle identifier" \
  || fail "Release.xcconfig must keep BUNDLE_IDENTIFIER = africa.cybota.cybercare.commons"
grep -q '^APP_DISPLAY_NAME = Cybercare Commons$' "$release_xcconfig" \
  && pass "Release.xcconfig keeps the production display name" \
  || fail "Release.xcconfig must keep APP_DISPLAY_NAME = Cybercare Commons"
...
grep -q 'resValue("string", "app_name", "Cybercare Commons")' "$gradle" \
  && pass "Gradle default app_name stays Cybercare Commons" \
