#!/usr/bin/env bash
# Uninstalls stale worktree-suffixed Commons debug builds from booted iOS
# simulators and connected Android devices/emulators. Production installs
# (africa.cybota.cybercare.commons, no suffix) are never touched:
...
ios_prefix="africa.cybota.cybercare.commons."
android_prefix="africa.cybota.cybercare.commons."
...
if [[ "$removed" == "0" ]]; then
    echo "no worktree-suffixed Commons installs found (production apps untouched)"
elif [[ "$dry_run" == "1" ]]; then
