...
test("boot splash overlay holds with a flapping bee, then dismisses", async ({
  ...
  // The mark is actually turning while the overlay holds — pure CSS, no SMIL.
  const spinState = await overlay
    .locator(".mark-spinner-outer")
    .evaluate((layer) => {
  ...
  expect(spinState).toEqual({ name: "mark-spin-cw", state: "running" });
