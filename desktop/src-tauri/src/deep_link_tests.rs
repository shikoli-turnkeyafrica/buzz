...
fn parse_entity_deep_link_accepts_every_share_link_shape() {
    ...
    for raw in golden["links"]
        ...
        .chain(golden["tabs"].as_array().unwrap().iter().map(|tab| {
            format!(
                "cybercare://repo?owner={owner}&d={dtag}&tab={}",
                tab.as_str().unwrap()
            )
        }))
    {
    ...
    let commit_link = format!(
        "cybercare://repo?owner={owner}&d={dtag}&tab=commits&commit={}",
...
fn parse_entity_deep_link_rejects_malformed_and_non_canonical_links() {
    ...
    for raw in [
        ...
        format!("cybercare://repo?owner={owner}"),
        "cybercare://repo?owner=nope&d=buzz-world".to_owned(),
        format!("cybercare://repo?owner={owner}&d=.hidden"),
        format!("cybercare://repo?owner={owner}&d=has%20space"),
        format!("cybercare://pr?owner={owner}&d=buzz-world"),
        format!("cybercare://pr?id=short&owner={owner}&d=buzz-world"),
        // Coordinate links take no event id.
        format!("cybercare://repo?id={event_id}&owner={owner}&d=buzz-world"),
        // Non-canonical: unknown param, duplicate param, path, fragment.
        format!("cybercare://repo?owner={owner}&d=buzz-world&relay=wss%3A%2F%2Fx.example"),
        format!("cybercare://repo?owner={owner}&owner={owner}&d=buzz-world"),
        // Unknown tab value, duplicate tab, and tab on an event link.
        format!("cybercare://repo?owner={owner}&d=buzz-world&tab=overview"),
        format!("cybercare://repo?owner={owner}&d=buzz-world&tab=prs&tab=prs"),
        format!("cybercare://repo?owner={owner}&d=buzz-world&tab=files&commit={event_id}"),
        format!("cybercare://repo?owner={owner}&d=buzz-world&tab=commits&commit=short"),
        format!("cybercare://pr?id={event_id}&owner={owner}&d=buzz-world&tab=prs"),
        format!("cybercare://repo/extra?owner={owner}&d=buzz-world"),
        format!("cybercare://repo?owner={owner}&d=buzz-world#top"),
        // Not an entity host.
        format!("cybercare://message?owner={owner}&d=buzz-world"),
    ] {
        assert!(
...
fn pending_entity_links_survive_until_acknowledged_in_order() {
    ...
    let first = queue.enqueue("cybercare://project?owner=aa&d=first".to_owned());
    let second = queue.enqueue("cybercare://project?owner=aa&d=second".to_owned());
...
fn pending_entity_links_dedupe_launch_and_open_callbacks() {
    let queue = PendingEntityDeepLinks::default();
    let href = "cybercare://project?owner=aa&d=buzz".to_owned();
...
fn parse_add_community_deep_link_extracts_relay_and_name() {
    let url = Url::parse(
        "cybercare://add-community?relay=wss%3A%2F%2Facme.communities.buzz.xyz&name=Acme%20Team&ignored=value",
    )
    .unwrap();
...
fn parse_add_community_deep_link_accepts_an_omitted_or_empty_name() {
    for raw in [
        "cybercare://add-community?relay=wss%3A%2F%2Facme.example",
        "cybercare://add-community?relay=wss%3A%2F%2Facme.example&name=",
    ] {
...
fn parse_add_community_deep_link_rejects_invalid_relays() {
    for raw in [
        "cybercare://add-community",
        "cybercare://add-community?relay=",
        "cybercare://add-community?relay=not-a-url",
        "cybercare://add-community?relay=https%3A%2F%2Facme.example",
        "cybercare://add-community?relay=wss%3A%2F%2F",
    ] {
...
fn parse_channel_deep_link_accepts_one_path_segment() {
    let url = Url::parse("cybercare://channel/580ca78b-9dae-46f3-8854-bd671853ba32").unwrap();
...
fn parse_channel_deep_link_accepts_message_path() {
    ...
    let url = Url::parse(&format!(
        "cybercare://channel/a372f080-5961-4535-b1a3-edffface377d/{message_id}"
    ))
    .unwrap();
    let payload = parse_channel_deep_link(&url).unwrap();
...
fn parse_channel_deep_link_rejects_malformed_forms() {
    for raw in [
        ...
        "cybercare://channel/580ca78b-9dae-46f3-8854-bd671853ba32/not-hex",
        "cybercare://channel/580ca78b-9dae-46f3-8854-bd671853ba32/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "cybercare://channel/580ca78b-9dae-46f3-8854-bd671853ba32/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/extra",
        "cybercare://channel/580ca78b-9dae-46f3-8854-bd671853ba32/",
        "cybercare://channel/one?extra=true",
        "cybercare://channel/one#fragment",
        "cybercare://:pass@channel/580ca78b-9dae-46f3-8854-bd671853ba32/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
...
fn parse_message_deep_link_extracts_required_params() {
    let url = Url::parse("cybercare://message?channel=abc&id=xyz").unwrap();
...
fn parse_message_deep_link_includes_thread_root() {
    let url = Url::parse("cybercare://message?channel=abc&id=xyz&thread=root1").unwrap();
    let payload = parse_message_deep_link(&url).expect("required params present");
...
fn parse_message_deep_link_rejects_missing_id() {
    let url = Url::parse("cybercare://message?channel=abc").unwrap();
...
fn parse_message_deep_link_rejects_empty_channel() {
    ...
    let url = Url::parse("cybercare://message?channel=&id=foo").unwrap();
    assert!(parse_message_deep_link(&url).is_none());
}

#[test]
fn parse_message_deep_link_rejects_empty_id() {
    let url = Url::parse("cybercare://message?channel=abc&id=").unwrap();
    assert!(parse_message_deep_link(&url).is_none());
}

#[test]
fn parse_message_deep_link_treats_empty_thread_as_absent() {
    let url = Url::parse("cybercare://message?channel=abc&id=xyz&thread=").unwrap();
    let payload = parse_message_deep_link(&url).expect("required params present");
    assert!(payload["threadRootId"].is_null());
}
...
fn parse_join_deep_link_extracts_relay_and_code() {
    let url = Url::parse("cybercare://join?relay=wss%3A%2F%2Frelay.example&code=abc.def").unwrap();
...
fn parse_join_deep_link_extracts_policy_receipt() {
    let url = Url::parse(
        "cybercare://join?relay=wss%3A%2F%2Frelay.example&code=abc.def&policy_receipt=receipt.value",
    )
    .unwrap();
    let payload = parse_join_deep_link(&url).expect("required params present");
...
fn parse_join_deep_link_rejects_missing_code() {
    let url = Url::parse("cybercare://join?relay=wss%3A%2F%2Frelay.example").unwrap();
    assert!(parse_join_deep_link(&url).is_none());
}

#[test]
fn parse_join_deep_link_rejects_empty_code() {
    let url = Url::parse("cybercare://join?relay=wss%3A%2F%2Frelay.example&code=").unwrap();
    assert!(parse_join_deep_link(&url).is_none());
}

#[test]
fn parse_join_deep_link_rejects_missing_relay() {
    let url = Url::parse("cybercare://join?code=abc.def").unwrap();
    assert!(parse_join_deep_link(&url).is_none());
}

#[test]
fn parse_join_deep_link_rejects_non_websocket_relay() {
    let url = Url::parse("cybercare://join?relay=https%3A%2F%2Frelay.example&code=abc.def").unwrap();
    assert!(parse_join_deep_link(&url).is_none());
}
...
fn parse_nostr_bind_deep_link_rejects_missing_challenge_id() {
    let url = Url::parse("cybercare://nostr-bind?nonce=ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghi01234567&verification_code=123456&audience=buzz%3Anostr-identity&action=bind_nostr_identity&protocol=buzz-nostr-identity&version=1&origin=https%3A%2F%2Fexample.com&expires_at=2999-01-01T00%3A00%3A00Z&return=clipboard").unwrap();
    assert!(parse_nostr_bind_deep_link(&url).is_err());
}
