...
fn identity_from_env() -> Option<Keys> {
    match std::env::var("BUZZ_PRIVATE_KEY") {
        Ok(nsec) => match Keys::parse(nsec.trim()) {
            ...
            Err(error) => {
                eprintln!(
                    "{}: invalid BUZZ_PRIVATE_KEY: {error}",
        ...
        Err(std::env::VarError::NotUnicode(_)) => {
            eprintln!(
                "{}: BUZZ_PRIVATE_KEY contains invalid UTF-8",
                crate::brand::LOG_PREFIX
            );
            None
        }
    ...
    match store.probe(IDENTITY_KEY_NAME) {
                    ...
                    Ok(keyring_keys) => {
                        ...
                        if legacy_path.exists() {
                            match load_key_file(legacy_path) {
                                ...
                                {
                                    eprintln!(
                                        "{}: identity.key differs from keyring; \
                                    ...
                                    let storage = if let Err(e) = persist_identity_to_keyring(
                                        ...
                                        eprintln!(
                                            "{}: keyring adoption of identity.key \
                                ...
                                Err(e) => {
                                    eprintln!(
                                        "{}: leftover identity.key is corrupt ({e}); \
                        ...
                        if !legacy_path.exists() && !migration_marker_path(data_dir).exists() {
                            ...
                            {
                                eprintln!(
                                    "{}: keyring present but marker missing; \
        ...
        KeyringProbe::ReachableButEmpty => {
            ...
            if legacy_path.exists() {
                ...
                eprintln!(
                    "{}: identity lost — keyring was empty despite migration marker; \
        ...
        KeyringProbe::Unreachable => {
            ...
            if !legacy_path.exists() && migration_marker_path(data_dir).exists() {
                let ephemeral = Keys::generate();
                eprintln!(
                    "{}: keyring unreachable but migration marker present; \
...
fn recover_from_keyring(
    ...
    eprintln!(
        "{}: corrupt nsec in keyring ({error}), clearing and recovering from file",
        crate::brand::LOG_PREFIX
    );
    if let Err(e) = store.delete(IDENTITY_KEY_NAME) {
        eprintln!(
            "{}: failed to clear corrupt keyring value: {e}",
            crate::brand::LOG_PREFIX
        );
    }
    if legacy_path.exists() {
    ...
    if migration_marker_path(data_dir).exists() {
        let ephemeral = Keys::generate();
        eprintln!(
            "{}: identity lost — keyring had corrupt data and no valid identity.key \
...
fn load_file_or_generate(
    ...
    eprintln!(
        "{}: generated and saved identity pubkey {}",
        crate::brand::LOG_PREFIX,
        keys.public_key().to_hex()
    );
    Ok(keys)
...
fn migrate_identity_file(
    ...
    let keys = match load_key_file(legacy_path) {
        ...
        Err(error) => {
            eprintln!(
                "{}: corrupt identity.key during migration ({error}), skipping",
                crate::brand::LOG_PREFIX
            );
            return Ok(None);
        }
    ...
    if let Err(e) = write_migration_marker(&marker_path) {
        eprintln!(
            "{}: keyring import ok but failed to write migration marker ({e}); \
    ...
    if let Err(e) = std::fs::remove_file(legacy_path) {
        eprintln!(
            "{}: keyring import ok but failed to delete identity.key: {e}",
            crate::brand::LOG_PREFIX
        );
    } else {
        eprintln!(
            "{}: migrated identity key into OS keyring",
            crate::brand::LOG_PREFIX
        );
    }
    Ok(Some(keys))
...
fn persist_identity_to_keyring(
    ...
    if let Err(e) = write_migration_marker(&marker_path) {
        ...
        if !legacy_path.exists() {
            if let Err(write_err) = save_key_file(legacy_path, keys) {
                eprintln!(
                    "{}: keyring ok but marker write failed ({e}) and \
                ...
                eprintln!(
                    "{}: keyring ok but marker write failed ({e}); \
    ...
    if legacy_path.exists() {
        if let Err(e) = std::fs::remove_file(legacy_path) {
            eprintln!(
                "{}: keyring write ok but failed to delete identity.key: {e}",
                crate::brand::LOG_PREFIX
            );
        }
    }
...
fn persist_imported_identity_impl(
    ...
    match persist_identity_to_keyring(store, keys, legacy_path, data_dir) {
        ...
        Err(e) => {
            eprintln!(
                "{}: keyring write failed during import ({e}), \
...
fn generate_and_persist(
    ...
    if storage == IdentityStorage::SystemKeyring {
        ...
        if let Err(e) = write_migration_marker(&marker_path) {
            eprintln!(
                "{}: stored identity in keyring but failed to write migration marker \
...
fn store_key_preferring_keyring(
    ...
    match store.store(IDENTITY_KEY_NAME, &nsec) {
        ...
        Err(keyring_err) => {
            eprintln!(
                "{}: keyring write failed ({keyring_err}), using file fallback",
                crate::brand::LOG_PREFIX
            );
            save_key_file(legacy_path, keys)?;
            Ok(IdentityStorage::LocalFile)
...
fn cleanup_leftover_identity_file(legacy_path: &std::path::Path) {
    ...
    match std::fs::remove_file(legacy_path) {
        Ok(()) => eprintln!(
            "{}: removed leftover identity.key (key is in keyring)",
            crate::brand::LOG_PREFIX
        ),
        Err(e) => eprintln!(
            "{}: failed to remove leftover identity.key: {e}",
            crate::brand::LOG_PREFIX
        ),
    }
}
...
fn quarantine_corrupt_key(key_path: &std::path::Path, data_dir: &std::path::Path, error: &str) {
    ...
    eprintln!(
        "{}: corrupt identity.key ({error}), quarantining to {bad_name}",
        crate::brand::LOG_PREFIX
    );
