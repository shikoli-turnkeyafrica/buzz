...
fn migrate_legacy_retention_into(
    ...
    match crate::managed_agents::retention::migrate_legacy_retention_db(
        ...
        Ok(copied) => {
            eprintln!(
                "{}: adopted {copied} legacy retained event(s) into this community",
                crate::brand::LOG_PREFIX
            )
        }
        Err(error) => eprintln!(
            "{}: legacy retention migration failed: {error}",
            crate::brand::LOG_PREFIX
        ),
    }
}
    ...
    #[cfg(feature = "mesh-llm")]
    {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if restore_pending {
                ...
                {
                    eprintln!(
                        "{}: failed to restore Share Compute: {error}",
                        crate::brand::LOG_PREFIX
                    );
                }
            }
            ...
            if restore_pending {
                ...
                {
                    eprintln!(
                        "{}: failed to restore managed agents: {error}",
                        crate::brand::LOG_PREFIX
                    );
                }
            }
