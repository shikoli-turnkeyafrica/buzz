...
impl PendingNavigationDeepLinks {
    fn lock(&self) -> std::sync::MutexGuard<'_, VecDeque<PendingNavigationDeepLink>> {
        self.0.lock().unwrap_or_else(|poisoned| {
            eprintln!(
                "{}: recovering poisoned pending navigation deep-link queue",
...
fn activate_main_window(app: &tauri::AppHandle) {
    ...
    if let Err(error) = window.unminimize() {
        eprintln!(
            "{}: failed to unminimize main window for deep link: {error}",
            crate::brand::LOG_PREFIX
        );
    }
    if let Err(error) = window.show() {
        eprintln!(
            "{}: failed to show main window for deep link: {error}",
            crate::brand::LOG_PREFIX
        );
    }
    if let Err(error) = window.set_focus() {
        eprintln!(
            "{}: failed to focus main window for deep link: {error}",
            crate::brand::LOG_PREFIX
        );
    }
}
...
pub(crate) fn install_deep_link_handlers(app: &mut tauri::App) {
    ...
    match app.deep_link().get_current() {
        ...
        Ok(None) => {}
        Err(error) => eprintln!(
            "{}: failed to read launch deep link: {error}",
            crate::brand::LOG_PREFIX
        ),
    }
}
...
pub(crate) fn handle_deep_link_url(app: &tauri::AppHandle, url_str: &str) {
    let url = match Url::parse(url_str) {
        ...
        Err(e) => {
            eprintln!(
                "{}: invalid deep link URL {url_str:?}: {e}",
                crate::brand::LOG_PREFIX
            );
            return;
        }
    ...
    if url.scheme() != crate::brand::DEEP_LINK_SCHEME {
        eprintln!(
            "{}: ignoring unsupported deep link scheme: {url_str}",
            crate::brand::LOG_PREFIX
        );
        return;
    }
    ...
    match url.host_str() {
        Some("connect") => {
            let Some(relay_url) = parse_websocket_relay_param(&url) else {
                eprintln!(
                    "{}: connect deep link missing/invalid relay: {url_str}",
                    crate::brand::LOG_PREFIX
                );
                return;
            };
        ...
        Some("join") => {
            ...
            let Some(payload) = parse_join_deep_link(&url) else {
                eprintln!(
                    "{}: join deep link missing/invalid relay or code: {url_str}",
                    crate::brand::LOG_PREFIX
                );
                return;
            };
        ...
        Some("add-community") => {
            let Some(payload) = parse_add_community_deep_link(&url) else {
                eprintln!(
                    "{}: add-community deep link missing/invalid relay: {url_str}",
                    crate::brand::LOG_PREFIX
                );
                return;
            };
        ...
        Some("channel") => {
            let Some(payload) = parse_channel_deep_link(&url) else {
                eprintln!(
                    "{}: channel deep link missing/invalid channel: {url_str}",
                    crate::brand::LOG_PREFIX
                );
                return;
            };
        ...
        Some("message") => {
            ...
            let Some(payload) = parse_message_deep_link(&url) else {
                eprintln!(
                    "{}: message deep link missing channel or id: {url_str}",
                    crate::brand::LOG_PREFIX
                );
                return;
            };
        ...
        Some("repo" | "project" | "pr" | "issue") => {
            ...
            if parse_entity_deep_link(&url).is_none() {
                eprintln!(
                    "{}: malformed entity deep link: {url_str}",
                    crate::brand::LOG_PREFIX
                );
                return;
            }
        ...
        Some("nostr-bind") => match parse_nostr_bind_deep_link(&url) {
            ...
            Err(error) => {
                eprintln!(
                    "{}: rejecting nostr-bind deep link: {error}: {url_str}",
                    crate::brand::LOG_PREFIX
                );
            }
        },
        Some(action) => {
            eprintln!(
                "{}: unknown deep link action: {action}",
                crate::brand::LOG_PREFIX
            );
        }
        None => {
            eprintln!(
                "{}: deep link missing action: {url_str}",
                crate::brand::LOG_PREFIX
            );
        }
    }
