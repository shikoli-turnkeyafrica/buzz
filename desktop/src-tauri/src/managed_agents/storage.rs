    ...
    match store.probe(&name) {
        ...
        KeyringProbe::Present | KeyringProbe::ReachableButEmpty => {
            match store.write_and_verify(&name, &record.private_key_nsec) {
                ...
                Err(e) => {
                    eprintln!(
                        "{}: keyring write for agent {} failed ({e}), keeping inline",
...
pub(crate) fn backup_invalid_store(path: &Path) {
    ...
    if let Err(e) = fs::copy(path, &backup) {
        eprintln!(
            "{}: failed to preserve malformed store {} as {}: {e}",
            crate::brand::LOG_PREFIX,
            path.display(),
            backup.display()
        );
    ...
    for record in records.iter_mut() {
        ...
        if record.private_key_nsec.is_empty() {
            match store.load(&agent_keyring_name(&record.pubkey)) {
                ...
                Ok(None) => {
                    eprintln!(
                        "{}: agent {} has no key in JSON or keyring",
                        crate::brand::LOG_PREFIX,
                        record.pubkey
                    );
                }
                ...
                Err(e) => {
                    eprintln!(
                        "{}: agent {} key unavailable — keyring read failed ({e}); \
...
/// (`"cybercare-commons"`) to the dev service (`"buzz-desktop-dev"`). Only
/// runs in debug builds — release builds never touch `"cybercare-commons"`
/// from this path.
///
...
pub fn migrate_agent_keys_to_dev_service(app: &tauri::AppHandle) {
    ...
    let records = match load_agent_store(app) {
        ...
        Err(e) => {
            eprintln!(
                "{}: keyring-dev-migration: cannot read agent store: {e}",
                crate::brand::LOG_PREFIX
            );
            return;
        }
    ...
    // singleton's cache.
    let prod_store = crate::secret_store::SecretStore::keyring(crate::brand::KEYRING_SERVICE);
...
fn copy_agent_keys_between_stores(pubkeys: &[String], src: &impl KeyStore, dst: &impl KeyStore) {
    ...
    if let Err(e) = dst.store_all(&to_write) {
        eprintln!(
            "{}: keyring-dev-migration: cannot write to dev keyring: {e}",
            crate::brand::LOG_PREFIX
        );
        return;
    }
    ...
    if copied > 0 {
        eprintln!(
            "{}: keyring-dev-migration: copied {copied} agent key(s) from buzz-desktop",
            crate::brand::LOG_PREFIX
        );
    }
}
...
pub fn delete_agent_key(pubkey: &str) {
    if let Err(e) = try_delete_agent_key(pubkey) {
        eprintln!(
            "{}: failed to delete agent {pubkey} key from keyring: {e}",
            crate::brand::LOG_PREFIX
        );
    }
}
