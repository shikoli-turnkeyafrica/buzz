...
export function getBuzzToolInfo(title: string): BuzzToolInfo | null {
  ...
  if (name.includes("workflow") || name === "approve_step") {
    return {
      icon: Workflow,
      label: isRead
        ? `Reads workflow state from ${PRODUCT_NAME}.`
        : `Updates workflow state in ${PRODUCT_NAME}.`,
  ...
  if (
    ...
    return {
      icon: Hash,
      label: isRead
        ? `Reads channel context from the ${PRODUCT_NAME} relay.`
        : `Changes channel state in the ${PRODUCT_NAME} relay.`,
      tone: isWrite ? "write" : "read",
    };
  }
  if (
    ...
    return {
      icon: Users,
      label: isRead
        ? `Reads ${PRODUCT_NAME} identity or presence data.`
        : `Updates ${PRODUCT_NAME} identity or membership data.`,
  ...
  if (name.includes("search") || name === "get_feed") {
    return {
      icon: Search,
      label: `Searches relay-visible ${PRODUCT_NAME} history.`,
      tone: "read",
    };
  }
  if (
    ...
    return {
      icon: Send,
      label: `Publishes relay-visible ${PRODUCT_NAME} activity.`,
      tone: "write",
    };
  }
  ...
  return {
    icon: MessageSquare,
    label: isRead
      ? `Reads from ${PRODUCT_NAME}.`
      : `Writes to ${PRODUCT_NAME}.`,
    tone: isWrite ? "write" : "read",
  };
}
