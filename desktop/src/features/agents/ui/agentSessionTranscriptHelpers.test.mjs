...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a "buzz event" (the wire-format prompt tag buzz-acp emits;
  // unrelated to the product brand), there is no event content to surface,
  // so userText is empty and the title falls through to "Commons event".
  ...
  assert.equal(result.userTitle, "Commons event");
...
test("parsePromptText defaults the title to 'Commons event' when no kind is present", () => {
