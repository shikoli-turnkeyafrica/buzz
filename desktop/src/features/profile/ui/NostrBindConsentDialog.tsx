...
const COPY_SUCCESS_MESSAGE = `Signed response copied. Paste it into the ${PRODUCT_NAME} admin console.`;
const PREVIEW_COPY_SUCCESS_MESSAGE = "Preview response copied.";
const COPY_FAILURE_MESSAGE = `${PRODUCT_NAME} couldn't access the clipboard. Try again.`;
    ...
    const unlistenPromise = listenForNostrBindDeepLinks((nextPayload) => {
      ...
      getIdentity()
        ...
        .catch((error) => {
          ...
          setIdentity(null);
          setError(`Could not load the current ${PRODUCT_NAME} identity.`);
        });
    });
                ...
                >
                  <DialogPrimitive.Title className="mt-6 text-3xl font-semibold tracking-tight">
                    {payload.returnMode === "browser_fragment_v1"
                      ...
                      : `Finish on the ${PRODUCT_NAME} website`}
                  ...
                  >
                    {payload.returnMode === "browser_fragment_v1"
                      ? `${PRODUCT_NAME} opened your browser to finish verification.`
                      : `Copy the response below, then paste it into the ${PRODUCT_NAME} website to finish verification.`}
                  </DialogPrimitive.Description>
