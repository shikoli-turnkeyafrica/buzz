...
export function HostedCommunityCreateFlow({
      ...
      if (bound.error) {
        ...
        throw new Error(
          bound.error.code === "pubkey_already_bound"
            ? `This device's ${PRODUCT_NAME} identity belongs to a different Builderlab account. Sign in with the account that already owns this identity.`
            : hostedCommunityErrorMessage(
                bound.error,
    ...
    return (
      <div className="space-y-5">
        <p className="text-sm leading-6 text-muted-foreground">
          Sign in with Builderlab to create and host a community. {PRODUCT_NAME}{" "}
  ...
  if (!identity) {
    return (
      <div className="space-y-5">
        <p className="text-sm leading-6 text-muted-foreground">
          Connect this device’s {PRODUCT_NAME} identity to your Builderlab
  ...
  if (identityMismatch) {
    return (
      <div className="space-y-5">
        <p className="text-sm leading-6 text-muted-foreground">
          This Builderlab account uses a different {PRODUCT_NAME} identity.
