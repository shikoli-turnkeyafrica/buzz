...
/**
 * Cybercare theme name. Cybercare is a first-party light theme that reuses
 * GitHub Light for every base color (backgrounds, text, borders, code) — the
 * message area and containers are indistinguishable from GitHub Light. Its
 * one distinguishing feature is a flat navy gradient painted across the
 * sidebar/nav canvas (the Cybota Cloud shell color), replacing GitHub
 * Light's flat grey. The gradient is applied by {@link ThemeProvider}
 * toggling a `data-buzz-sidebar` attribute on the document root; the CSS
 * lives in `shared/styles/globals/theme.css`. That attribute (and its
 * sibling `buzz-*` DOM hooks) intentionally keeps its `buzz` name — see the
 * comment in theme.css.
 */
export const CYBERCARE_THEME_NAME = "cybercare";

/**
 * Cybercare Dark theme name. The dark-mode counterpart to
 * {@link CYBERCARE_THEME_NAME}: reuses the GitHub Dark palette for every base
 * color, with the same branded navy sidebar gradient (dark-tuned colors, see
 * `shared/styles/globals/theme.css`). {@link ThemeProvider} toggles the
 * shared `data-buzz-sidebar` attribute for this theme too; the `.dark` root
 * class selects the dark gradient values.
 *
 * Cybercare and Cybercare Dark are paired in {@link THEME_PAIRS}, so the
 * picker shows a combined "Cybercare" tile under System mode (follow-OS)
 * plus a single "Cybercare" tile under Light and a "Cybercare Dark" tile
 * under Dark.
 */
export const CYBERCARE_DARK_THEME_NAME = "cybercare-dark";

/** The Shiki bundle Cybercare borrows its base palette from. */
export const CYBERCARE_BASE_THEME: SyntaxThemeName = "github-light";

/** The Shiki bundle Cybercare Dark borrows its base palette from. */
export const CYBERCARE_DARK_BASE_THEME: SyntaxThemeName = "github-dark";

/**
 * Resolve a theme name to the real Shiki bundled theme it maps to.
 *
 * Most themes map to themselves, but the Cybercare aliases (`cybercare` /
 * `cybercare-dark`) are not bundled Shiki themes — they reuse the GitHub
 * Light / GitHub Dark palettes. The Shiki highlighter engine (used for
 * fenced code blocks in `CodeBlock.tsx`) only understands bundled names, so
 * callers that hand a theme name to `loadTheme` / `codeToTokens` must
 * resolve it through here first; passing a raw Cybercare alias makes Shiki
 * throw and code blocks fall back to unhighlighted plain text.
 */
export function resolveShikiThemeName(name: string): SyntaxThemeName {
  if (name === CYBERCARE_THEME_NAME) return CYBERCARE_BASE_THEME;
  if (name === CYBERCARE_DARK_THEME_NAME) return CYBERCARE_DARK_BASE_THEME;
  return name as SyntaxThemeName;
}

// Available themes. "cybercare" is a Cybercare-branded theme that reuses the
// github-light palette plus a navy sidebar gradient; the rest are the Shiki
...
> = {
  // Cybercare reuses the github-light palette; its gradient is applied separately.
  cybercare: () => import("shiki/themes/github-light.mjs"),
  // Cybercare Dark reuses the github-dark palette; dark gradient applied separately.
  "cybercare-dark": () => import("shiki/themes/github-dark.mjs"),
...
export const THEME_PAIRS: ReadonlyMap<SyntaxThemeName, SyntaxThemeName> =
  new Map([
    // Light → Dark
    // Cybercare is the first-party pair; keep it first so it leads every category.
