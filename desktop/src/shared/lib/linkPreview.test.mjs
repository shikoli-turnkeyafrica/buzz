...
test("parseSupportedLinkPreview parses cybercare:// PR and issue deep links", () => {
  assert.deepEqual(
    parseSupportedLinkPreview(
      `cybercare://pr?id=${BUZZ_EVENT_ID}&owner=${BUZZ_OWNER}&d=buzz-world`,
    ),
    {
  ...
  assert.deepEqual(
    parseSupportedLinkPreview(
      `cybercare://issue?id=${BUZZ_EVENT_ID}&owner=${BUZZ_OWNER}&d=buzz-world`,
    )?.typeLabel,
    "Task",
  );
  assert.deepEqual(
    parseSupportedLinkPreview(
      `cybercare://repo?owner=${BUZZ_OWNER}&d=buzz-world`,
    ),
    {
      kind: "buzz-repository",
      href: `cybercare://repo?owner=${BUZZ_OWNER}&d=buzz-world`,
...
test("parseSupportedLinkPreview parses cybercare:// project deep links", () => {
  assert.deepEqual(
    parseSupportedLinkPreview(
      `cybercare://project?owner=${BUZZ_OWNER}&d=buzz-world`,
    ),
    {
      kind: "buzz-project",
...
test("parseSupportedLinkPreview rejects malformed cybercare:// entity links", () => {
  for (const href of [
    `cybercare://pr?owner=${BUZZ_OWNER}&d=buzz-world`,
    `cybercare://pr?id=short&owner=${BUZZ_OWNER}&d=buzz-world`,
    `cybercare://issue?id=${BUZZ_EVENT_ID}&owner=nope&d=buzz-world`,
    `cybercare://repo?owner=${BUZZ_OWNER}&d=.hidden`,
    `cybercare://project?owner=${BUZZ_OWNER}&d=.hidden`,
  ]) {
    assert.equal(parseSupportedLinkPreview(href), null, href);
  }
});

test("extractSupportedLinkPreviews picks up cybercare:// project links in prose", () => {
  assert.deepEqual(
    extractSupportedLinkPreviews(
      `tracking here: cybercare://project?owner=${BUZZ_OWNER}&d=buzz-world`,
    ).map((preview) => [preview.kind, preview.typeLabel, preview.title]),
    [["buzz-project", "project", "buzz-world"]],
  );
});

test("extractSupportedLinkPreviews picks up cybercare:// links in prose", () => {
  assert.deepEqual(
    extractSupportedLinkPreviews(
      `PR is up: cybercare://pr?id=${BUZZ_EVENT_ID}&owner=${BUZZ_OWNER}&d=buzz-world — review please.`,
    ).map((preview) => [preview.kind, preview.title]),
    [["buzz-pull-request", "buzz-world #c3b589fa"]],
  );
});

test("extractSupportedLinkPreviews uses markdown labels for cybercare:// links", () => {
  assert.deepEqual(
    extractSupportedLinkPreviews(
      `[Add header links](cybercare://pr?id=${BUZZ_EVENT_ID}&owner=${BUZZ_OWNER}&d=buzz-world)`,
...
test("extractSupportedLinkPreviews dedupes clone URL variants of one repo", () => {
  assert.deepEqual(
    ...
    [`cybercare://repo?owner=${BUZZ_OWNER}&d=buzz-world-galaxy`],
  );
});

test("clone URLs and cybercare://repo links for the same repo dedupe to one card", () => {
  assert.deepEqual(
    extractSupportedLinkPreviews(
      [
        `https://relay.example/git/${BUZZ_OWNER}/buzz-world-galaxy`,
