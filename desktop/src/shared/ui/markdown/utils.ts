...
/**
 * `urlTransform` for `<ReactMarkdown>` that preserves `cybercare://` deep links
 * used by Buzz — both `cybercare://message?…` links and `cybercare://pr|issue|repo?…`
 ...
 *
 * Policy:
 * - `cybercare://message` hrefs — preserved unconditionally (handled by the
 *   message-link pill renderer).
 * - `cybercare://pr|issue|repo` hrefs — preserved only when `parseEntityLink`
 *   succeeds, keeping the sanitizer active against arbitrary `cybercare://` URIs.
 ...
 * which also handles `cybercare://pr|issue|repo` entity links.
 */
